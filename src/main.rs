        ...
        if Path::new(&accountsservice_path).exists() {
            let _ = Command::new("sudo")
                .args([
                    "sed",
                    "-i",
                    &std::format!("s/^XSession=.*/XSession={}/", session_name),
...
fn create_profile(
    ...
    let _config_files = collect_config_files(config);
    ...
    ui.on_edit_profile(move |index| {
        ...
        if let Some(profile) = profiles.get(index as usize) {
            if let Some(ui) = ui_weak.upgrade() {
                ui.set_status_message(
                    std::format!(
                        "Profile: {} | DE: {} | Created: {} | Packages: {} | Remote: {}",
                        profile.name,
                        profile.desktop_environment,
                        profile.created,
                        profile.packages.len(),
                        if profile.is_remote { "Yes" } else { "No" }
                    )
                    .into(),
                );
                ui.set_status_type("info".into());
